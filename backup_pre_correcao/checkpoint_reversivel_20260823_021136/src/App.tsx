...
export default function App() {
  ...
  const syncDataFromCloud = async (userId: string) => {
    ...
    try {
      ...
      if (INCREMENTAL_SYNC_ENABLED && await isIncrementalSyncReady(userId)) {
      ...
      if (!hasCloud) {
        ...
        if (CHANGE_AWARE_SYNC_ENABLED || INCREMENTAL_SYNC_ENABLED) await rememberCloudSyncVersion(userId);
