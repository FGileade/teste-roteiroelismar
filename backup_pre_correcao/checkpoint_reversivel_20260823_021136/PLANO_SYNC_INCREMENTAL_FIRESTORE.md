...
**Status:** implementação local parcial concluída, com flags desligadas; nenhuma publicação ou alteração de dados de produção executada.
...
- O backfill, o piloto e o deploy continuam pendentes.
...
**Aprovado com ressalvas para preparação local; bloqueado para deploy e ativação neste momento.**

Ainda faltam restauração de backup em ambiente descartável, backfill validado, validação de conflitos, piloto com usuário interno, comparação de métricas e confirmação dos custos de backup antes de ativar a mudança em produção.
