...
type TabType = 'agenda' | 'clientes' | 'configuracoes';
...
export default function App() {
  ...
  const syncWithCloud = async <T extends { id: string }>(
    ...
    if (!currentUser || isSyncingWithCloud) return;
    try {
      setWriteError(null);
      await syncArrayToCloud(resolveUserId(currentUser.uid), collectionName, oldItems, newItems);
    } catch (err: any) {
      console.error(err);
  ...
  const saveClientsAndVisitsToStorage = (updatedClients: Client[], updatedVisits: Visit[]) => {
    ...
    if (currentUser) {
      syncWithCloud('clients', clients, updatedClients);
      syncWithCloud('visits', visits, updatedVisits);
    }
  };
  ...
  const saveNegotiationsToStorage = (updatedNegs: NegotiationHistory[]) => {
    ...
    if (currentUser) {
      syncWithCloud('negotiations', negotiations, updatedNegs);
    }
  };
  ...
  const saveVoiceNotesToStorage = (updatedNotes: VoiceNote[]) => {
    ...
    if (currentUser) {
      syncWithCloud('voiceNotes', voiceNotes, updatedNotes);
    }
  };
  ...
  const saveEventsToStorage = (updatedEvents: AgendaEvent[]) => {
    ...
    if (currentUser) {
      syncWithCloud('events', agendaEvents, updatedEvents);
    }
  };
  ...
  const saveLoansToStorage = (updatedLoans: ProductLoan[]) => {
    ...
    if (currentUser) {
      syncWithCloud('loans', loans, updatedLoans);
    }
  };
