...
import { CHANGE_AWARE_SYNC_ENABLED, isIncrementalSyncEnabledForUser } from './firebaseSync';
...
export async function restoreBackup(
  ...
  if (CHANGE_AWARE_SYNC_ENABLED || isIncrementalSyncEnabledForUser(userId)) {
    await setDoc(doc(db, 'users', userId, 'config', 'syncState'), {
      version: increment(1),
      incrementalReady: isIncrementalSyncEnabledForUser(userId),
      schemaVersion: isIncrementalSyncEnabledForUser(userId) ? 1 : 0,
...
async function restoreCollection(
  ...
  for (const item of items) {
    ...
    batch.set(ref, isIncrementalSyncEnabledForUser(userId)
      ? {
          ...item,
          _sync: {
