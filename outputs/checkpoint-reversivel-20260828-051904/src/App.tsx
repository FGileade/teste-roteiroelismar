...
export default function App() {
  ...
  const queueChangesForUser = <T extends { id: string }>(
    userId: string,
  ...
  const queueChangesForStoredUser = <T extends { id: string }>(
    collectionName: string,
    oldItems: T[],
    newItems: T[],
  ) => {
    ...
    if (storedUserId) {
      queueChangesForUser(resolveUserId(storedUserId), collectionName, oldItems, newItems);
    }
  };
  ...
  const syncWithCloud = async <T extends { id: string }>(
    ...
    try {
      ...
      queueChangesForUser(userId, collectionName, oldItems, newItems);
  ...
  const saveClientsAndVisitsToStorage = (updatedClients: Client[], updatedVisits: Visit[]) => {
    ...
    if (currentUser) {
      ...
      queueChangesForStoredUser('clients', oldClients, updatedClients);
      queueChangesForStoredUser('visits', oldVisits, updatedVisits);
    }
  };
  ...
  const saveNegotiationsToStorage = (updatedNegs: NegotiationHistory[]) => {
    ...
    if (currentUser) {
      ...
      queueChangesForStoredUser('negotiations', oldNegs, updatedNegs);
    }
  };
  ...
  const saveVoiceNotesToStorage = (updatedNotes: VoiceNote[]) => {
    ...
    if (currentUser) {
      ...
      queueChangesForStoredUser('voiceNotes', oldNotes, updatedNotes);
    }
  };
  ...
  const saveEventsToStorage = (updatedEvents: AgendaEvent[]) => {
    ...
    if (currentUser) {
      ...
      queueChangesForStoredUser('events', oldEvents, updatedEvents);
    }
  };
  ...
  const saveLoansToStorage = (updatedLoans: ProductLoan[]) => {
    ...
    if (currentUser) {
      ...
      queueChangesForStoredUser('loans', oldLoans, updatedLoans);
    }
  };
