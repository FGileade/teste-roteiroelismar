...
O inventário abaixo foi conferido no código de referência local. A confirmação visual do projeto Stitch não foi possível nesta sessão porque não havia navegador disponível; portanto, a preservação das telas e a identificação do projeto Stitch permanecem pendentes de conferência manual.

| Tela ou fluxo | Dispositivo | Finalidade | Situação da referência local |
|---|---|---|---|
| Login | Mobile e desktop | Entrada e autenticação visual | Existente |
| Agenda / Dashboard | Mobile, tablet e desktop | Roteiro diário, métricas e status de sincronização | Existente |
| Detalhe da visita | Mobile, tablet e desktop | Consultar cliente, endereço e histórico | Existente |
| Concluir visita | Mobile, tablet e desktop | Registrar observações e resultado | Existente |
| Cancelar ou reagendar visita | Mobile, tablet e desktop | Registrar o desfecho da visita | Existente |
| Visita extra | Mobile, tablet e desktop | Adicionar cliente à agenda do dia | Existente |
| Eventos e compromissos | Mobile, tablet e desktop | Criar e consultar lançamentos da agenda | Existente |
| Anotações por voz | Mobile, tablet e desktop | Registrar e vincular anotações ao cliente | Existente |
| Carteira de clientes | Mobile, tablet e desktop | Pesquisar, filtrar e consultar clientes | Existente |
| Novo ou editar cliente | Mobile, tablet e desktop | Manter dados cadastrais e rota | Existente |
| Empréstimos | Mobile, tablet e desktop | Consultar, criar, devolver e gerar relatório | Existente |
| Configurações e perfil | Mobile, tablet e desktop | Gerenciar acesso, endereço, tela e dados | Existente |
| Navegação principal | Mobile, tablet e desktop | Alternar entre Agenda, Clientes, Empréstimos e Configurações | Existente |

#### Verificações realizadas

- A aplicação usa `lang="pt-BR"` e os textos da interface estão predominantemente em português do Brasil.
- Não foram criadas nem removidas telas ou integrações durante esta etapa.
- Os dados iniciais do código são dados de demonstração; novas telas do Stitch devem continuar usando dados fictícios e identificados como demonstrativos.
- O código de produção já possui integrações Firebase, ViaCEP, mapas, Waze e WhatsApp. Elas não foram alteradas nem devem ser reproduzidas no protótipo visual do Stitch.
- A ordem operacional permanece: clientes e empréstimos → visita extra e estados de visitas → erros de pedidos e sincronização → ajuda e suporte → responsividade e auditoria final.

#### Pendências para concluir a Etapa 0

- Confirmar visualmente o projeto Stitch `13959033504928489482`.
- Conferir se todas as telas listadas estão preservadas no canvas.
- Validar manualmente os breakpoints de 320, 390, 768 e 1440 pixels.
- Registrar no inventário qualquer nova tela criada no Stitch com nome, dispositivo e finalidade.
...
|---|---|---|
| 0. Preparação | Parcialmente concluída | Inventário local atualizado; confirmação visual do Stitch pendente |
