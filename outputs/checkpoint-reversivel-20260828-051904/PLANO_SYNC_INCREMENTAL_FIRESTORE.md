...
**Status:** piloto incremental publicado para o UID Master; ativação ampla ainda bloqueada por métricas, conflitos e validações de segurança.

## Status verificado em 23/08/2026

- `downloadUserData` continua disponível como fallback seguro.
- O fluxo incremental foi implementado em `src/lib/firebaseSync.ts`, com cursor por coleção, paginação de 200 documentos, tombstones e marcador de versão.
- A fila de `src/lib/syncQueue.ts` grava metadados incrementais somente quando `VITE_FIRESTORE_INCREMENTAL_SYNC=true`.
- `hasCloudData()` agora propaga erros e nunca trata falha de rede/permissão como banco vazio.
- As regras locais foram restringidas aos UIDs autorizados e passaram em 7/7 testes do Emulator.
- `npm run lint` e `npm run build` passaram.
- `npm run test:incremental` passou com 3/3 casos no Emulator: cursor estável, tombstone e bloqueio de conflito.
- O backup protegido foi restaurado com sucesso no Firestore Emulator, preservando contagens e IDs.
- O backfill real foi concluído para 662 documentos após o backup validado.
- O piloto foi publicado com allowlist somente para o UID Master e validado com 122 clientes carregados.
- A proteção transacional de `baseRevision` foi publicada no piloto e passou no smoke test pós-deploy.
- O último deploy executado foi somente do Hosting; regras e índices precisam ser confirmados/publicados no Firestore antes da ativação ampla.
- A ativação ampla, a medição de Billing e a validação de conflitos em dois dispositivos continuam pendentes; a proteção de revisão já passou no Emulator.
...
- Implementado localmente `downloadIncrementalUserData` sem remover `downloadUserData`; a flag permanece desligada.
...
- Metadados e tombstones foram implementados na fila, atrás da flag incremental.
...
- `hasCloudData` continua consultando somente `clients` com `limit(1)`, mas erros agora são propagados e não iniciam upload indevido.
- `uploadCollection` permanece reservado ao bootstrap/importação; o fluxo normal usa a fila offline.
...
**Piloto publicado com ressalvas; ativação ampla bloqueada até concluir métricas, conflitos, regras e observação de estabilidade.**

Ainda faltam validação de conflitos, comparação de métricas, observação do Billing e confirmação dos custos de backup antes de ampliar a mudança para os demais usuários.
