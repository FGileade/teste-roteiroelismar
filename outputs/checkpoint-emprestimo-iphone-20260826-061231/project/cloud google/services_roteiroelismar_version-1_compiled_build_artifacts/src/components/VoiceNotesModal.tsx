...
export default function VoiceNotesModal({
  ...
  const toggleListening = () => {
    if (!recognitionRef.current) {
      alert('O reconhecimento de voz não é suportado pelo seu navegador atual ou está bloqueado nas configurações de privacidade. Por favor, digite ou use o botão de simular!');
      return;
    }
            ...
            <div className="space-y-4 flex flex-col h-full justify-between">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-center gap-3 py-1">
                  ...
                  {!isListening && (
                    <button
                      id="btn_simulate_voice"
                      type="button"
                      onClick={handleSimulateVoice}
                      className="px-3 py-2 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 rounded-xl text-[10px] font-extrabold uppercase transition-colors"
                      title="Simula uma transcrição de voz para testes no frame"
                    >
                      💡 Simular Voz
                    </button>
                  )}
