...
export default function ClientManagement({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !buyerName.trim() || !phone.trim() || !street.trim() || !number.trim() || !city.trim() || !state.trim()) {
      alert('Por favor, preencha todos os campos obrigatórios (*), incluindo Rua, Número, Cidade e Estado.');
      return;
    }

    // Compile complete address string for storage
    const compiledAddress = `${street.trim()}, ${number.trim()} - ${city.trim()} - ${state.trim()}${cep ? `, ${cep.trim()}` : ''}`;
