...
const LOCAL_SYNC_COLLECTIONS = ['clients', 'visits', 'negotiations', 'voiceNotes', 'events', 'loans'] as const;
...
export function isIncrementalSyncEnabledForUser(userId: string): boolean {
...
/**
 * Clears the local incremental-sync checkpoint for a user.
 *
 * The application clears the local datasets on logout. Keeping the marker or
 * cursors after that would make the next login incorrectly believe that the
 * empty local state was already synchronized.
 */
export function clearLocalSyncState(userId: string): void {
  if (!userId) return;

  localStorage.removeItem(localSyncVersionKey(userId));
  for (const collectionName of LOCAL_SYNC_COLLECTIONS) {
    localStorage.removeItem(localCursorKey(userId, collectionName));
  }
}
...
export async function uploadCollection<T extends { id: string }>(
  ...
  try {
    ...
    for (const item of items) {
      ...
      writeBatchInstance.set(docRef, isIncrementalSyncEnabledForUser(userId)
        ? {
            ...item,
            _sync: {
              revision: increment(1),
...
export async function saveInitializedDatesToCloud(userId: string, dates: string[]) {
  try {
    const configDoc = doc(db, 'users', userId, 'config', 'dates');
    if (CHANGE_AWARE_SYNC_ENABLED || isIncrementalSyncEnabledForUser(userId)) {
...
export async function syncArrayToCloud<T extends { id: string }>(
  ...
  try {
    ...
    if (toSet.length + toDelete.length === 1) {
      ...
      if (toSet.length === 1) {
        const item = toSet[0];
        batch.set(doc(colRef, item.id), isIncrementalSyncEnabledForUser(userId)
          ? {
              ...item,
              _sync: {
                revision: increment(1),
